 ...
 *****************************************************************************/
/*
This module provides methods for the generalized mesh primitive gob.
*/
...
static void traverselights( P_Transform *thistrans, P_Attrib_List *thisattr )
{
  ...
  ger_debug("mesh_mthd: traverselights");
  /* Do nothing */
				  ...
				  P_Attrib_List *thisattr )
{
  ...
  ger_debug("mesh_mthd: traverselights_to_ren");
  /* Do nothing */
...
static void render( P_Transform *thistrans, P_Attrib_List *thisattr )
{
  ...
  ger_debug("mesh_mthd: render");
  thisrenlist= RENLIST(self);
  while (thisrenlist) {
    METHOD_RDY(RENDERER(thisrenlist));
    (*(RENDERER(thisrenlist)->ren_mesh))
...
/* Render to the named renderer only */
{
  ...
  ger_debug("mesh_mthd: render_to_ren");
...
/* This method returns the data given by the renderer at definition time */
{
  ...
  ger_debug("mesh_mthd: get_ren_data");
  thisrenlist= RENLIST(self);
  while (thisrenlist) {
    if ( RENDERER(thisrenlist) == thisrenderer ) {
      METHOD_OUT
...
/* This method defines the gob within the context of the given renderer */
{
  ...
  ger_debug("mesh_mthd: define_self");
  ...
  if ( !(thisrenlist= (P_Ren_List *)malloc(sizeof(P_Ren_List))) )
    ger_fatal("mesh_mthd: define_gob: unable to allocate %d bytes!",
              sizeof(P_Ren_List) );
  ...
  RENDATA(thisrenlist)= (*(thisrenderer->def_mesh))
    (self->name, DATA(self)->vlist, DATA(self)->indices, 
     DATA(self)->facet_lengths, DATA(self)->nfacets);
...
/* This is the print method for the gob. */
{
  ...
  int facet, vertex, index;
  METHOD_IN

  ger_debug("mesh_mthd: print");
  ind_write("Gob <%s>:  mesh",
	    self->name); 
  ind_eol();
  ind_push();
  ind_write("vertices:"); ind_eol();
  ind_push();
  METHOD_RDY(DATA(self)->vlist);
  (*(DATA(self)->vlist)->print)();
  ind_pop();
  index= 0;
  for (facet=0; facet<DATA(self)->nfacets; facet++) {
    ind_write("facet %d indices: ",facet);
    for (vertex=0; vertex<DATA(self)->facet_lengths[facet]; vertex++)
      ind_write("%d ",DATA(self)->indices[index++]);
    ind_eol();
    }
  ind_pop();
 ...
 */
{
  ...
  if ( (self->held) || (self->parents) ) {
    ger_debug("mesh_mthd: destroy: held or with parents; not destroyed");
    METHOD_OUT
    return;
  }

  ger_debug("mesh_mthd: destroy");

  /* destroy renderer rep of this gob */
  if (destroy_ren_rep) WALK_RENLIST(self,destroy_mesh);
...
P_Gob *po_create_mesh( char *name, P_Vlist *vlist, int *indices, 
		      int *facet_lengths, int nfacets)
/* This function returns a generalized mesh primitive gob.  Other methods
 * are provided by po_create_primitive. 
 */
{
  P_Gob *thisgob;
  P_Ren_Data *thisrendata;

  ger_debug("po_create_mesh: name= <%s>", name);
  ...
  if ( !(thisrendata= (P_Ren_Data *)malloc(sizeof(P_Ren_Data))) )
    ger_fatal("mesh_mthd: po_create_mesh: unable to allocate %d bytes!",
              sizeof(P_Ren_Data) );
  thisgob->object_data= (P_Void_ptr)thisrendata;
  RENLIST(thisgob)= (P_Ren_List *)0;
  DATA(thisgob)->vlist= vlist;
  DATA(thisgob)->indices= indices;
  DATA(thisgob)->facet_lengths= facet_lengths;
  DATA(thisgob)->nfacets= nfacets;

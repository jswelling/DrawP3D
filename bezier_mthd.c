 ...
 *****************************************************************************/
/*
This module provides methods for the bezier patch primitive gob.
*/
...
static void traverselights( P_Transform *thistrans, P_Attrib_List *thisattr )
{
  ...
  ger_debug("bezier_mthd: traverselights");
  /* Do nothing */
				  ...
				  P_Attrib_List *thisattr )
{
  ...
  ger_debug("bezier_mthd: traverselights_to_ren");
  /* Do nothing */
...
static void render( P_Transform *thistrans, P_Attrib_List *thisattr )
{
  ...
  ger_debug("bezier_mthd: render");
  thisrenlist= RENLIST(self);
  while (thisrenlist) {
    METHOD_RDY(RENDERER(thisrenlist));
    (*(RENDERER(thisrenlist)->ren_bezier))
...
/* Render to the named renderer only */
{
  ...
  ger_debug("bezier_mthd: render_to_ren");
...
/* This method returns the data given by the renderer at definition time */
{
  ...
  ger_debug("bezier_mthd: get_ren_data");
...
/* This method defines the gob within the context of the given renderer */
{
  ...
  ger_debug("bezier_mthd: define_self");
  ...
  if ( !(thisrenlist= (P_Ren_List *)malloc(sizeof(P_Ren_List))) )
    ger_fatal("bezier_mthd: define_gob: unable to allocate %d bytes!",
              sizeof(P_Ren_List) );
  ...
  RENDATA(thisrenlist)= (*(thisrenderer->def_bezier))
    (self->name, DATA(self)->vlist);
...
/* This is the print method for the gob. */
{
  ...
  ger_debug("bezier_mthd: print");
  ind_write("Gob <%s>:  bezier",
	    self->name); 
  ind_eol();
  ind_push();
 ...
 */
{
  ...
  if ( (self->held) || (self->parents) ) {
    ger_debug("bezier_mthd: destroy: held or with parents; not destroyed");
    METHOD_OUT
    return;
  }

  ger_debug("bezier_mthd: destroy");

  /* destroy renderer rep of this gob */
  if (destroy_ren_rep) WALK_RENLIST(self,destroy_bezier);
...
P_Gob *po_create_bezier( char *name, P_Vlist *vlist )
/* This function returns a bezier primitive gob.  Other methods are
 ...
 */
{
  ...
  ger_debug("po_create_bezier: name= <%s>", name);

  /* Test to see if we have the right number of vertices */
  if (vlist->length != 16) {
    ger_error("bezier_mthd: po_create_bezier: \n\
need 16 vertices to define a Bezier patch; got %d",vlist->length);
    return( (P_Gob *)0 );
  }
  ...
  if ( !(thisrendata= (P_Ren_Data *)malloc(sizeof(P_Ren_Data))) )
    ger_fatal("bezier_mthd: po_create_bezier: unable to allocate %d bytes!",
              sizeof(P_Ren_Data) );
